...
export const checkMatchPath = (field: GeneralField) => {
  isField(field) &&
    field.setValidator(value => {
      try {
        const group = [String(field.query(".path").value() || ""), String(value || "")].filter(
          value => (value ? FormPath.parse(value).isMatchPattern : false),
        );
...
export const matchEffect = (reactFilter?: FilterFn) => {
  ...
  onFieldReact("group.*.print", field => {
    ...
    try {
      const [address = "", value = "", additional = ""] = reactFilter
        ? reactFilter([path, text, field])
        : [path, text];
      if (address && value) {
