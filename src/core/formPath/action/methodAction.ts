...
const bothPathLimit = (field: GeneralField, path: string) => {
  ...
  const group = [value, path].filter(value =>
    value ? FormPath.parse(value).isMatchPattern : false,
  );
...
const nodeValue = (field: GeneralField, name: string) => {
  const array: string[] = [];
  field
    .query(`.${name}.${name}-array.*`)
    .forEach(item => isField(item) && !!item.value && array.push(item.value));
...
const spliceValue = (field: GeneralField, path?: string) => {
  ...
  field
    .query(`${path || ""}.splice-index.*`)
    .forEach(item => isField(item) && index.push(parseInt(item.value) || 0));
  field
    .query(`${path || ""}.splice-array.*`)
    .forEach(item => isField(item) && !!item.value && array.push(item.value));
...
FormPath.parse('${path}').getIn(target)`,
  ...
  action_matchAliasGroup: (field, path) => {
    const [[pattern = "", alias = ""], feedback] = groupPathLimit(
      field,
      ".matchAliasGroup.matchAliasGroup-array.*",
    );
    field.setDecoratorProps({
...
const fieldMethod: Record<string, ActionMethod> = {
  ...
  action_deleteIn: (field, path) => {
    isField(field) &&
      field.query(".deleteIn-input").take(target => {
        if (isField(target)) {
  ...
  action_existIn: (field, path) => {
    isField(field) &&
      field.query(".existIn-input").take(target => {
        if (isField(target)) {
          const [data, feedback] = jsonLimit(target.value);
  ...
  action_getIn: (field, path) => {
    isField(field) &&
      field.query(".getIn-input").take(target => {
        if (isField(target)) {
          const [data, feedback] = jsonLimit(target.value);
          field.value = feedback || JSON.stringify(FormPath.parse(path).getIn(data));
        }
  ...
  action_includes: (field, path) => {
    isField(field) &&
      field.query(".includes-input").take(input => {
        const [subpath, feedback] = subPathLimit(input);
        if (feedback === "") {
  ...
  action_map: (field, path) => {
    const target = dataPathLimit(field, path);
    if (target !== void 0) {
      target.value = FormPath.parse(path)
        .map(key => `${key}~`)
        .toString();
    }
  },
  action_match: (field, path) => {
    isField(field) &&
      field.query(".match-input").take(input => {
        const [value, feedback] = bothPathLimit(input, path);
  ...
  action_matchAliasGroup: (field, path) => {
    if (isField(field)) {
      const [[pattern = "", alias = ""], feedback] = groupPathLimit(
        field,
        ".matchAliasGroup-array.*",
      );
      field.value = feedback
        ? feedback.replace("子", "匹配")
        : FormPath.parse(path).matchAliasGroup(pattern, alias)
          ? "true"
          : "false";
    }
  },
