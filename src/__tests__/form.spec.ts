...
test("no field initialValues merge", () => {
  const form = attach(
    createForm<{ aa: string; bb?: string }>({
      values: { aa: "123" },
...
test("deleteValuesIn/deleteInitialValuesIn", () => {
  const form = attach(
    createForm<Record<string, number>>({
...
test("reset", async () => {
  const form = attach(
    createForm<Partial<Record<"aa" | "bb", number | string>>>({
      initialValues: { aa: 123 },
      values: { bb: 123 },
    }),
  );
...
test("initialValues merge values after create field", () => {
  ...
  const array = attach(form.createArrayField({ name: "array" }));
  const arr_0_aa = attach(
    form.createField({ basePath: "array.0", initialValue: "123", name: "aa" }),
  );
...
test("form lifecycle can be triggered after call form.setXXX", () => {
  const form = attach(
    createForm<Partial<Record<"aa" | "bb", number>>>({
      initialValues: { aa: 1 },
      values: { bb: 1 },
    }),
  );
...
test("validate will skip display none", async () => {
  ...
  // 修复后：用 rejects 匹配器替代 try/catch，消除条件性 expect
  await expect(form.validate()).rejects.toEqual(defaultError);
  ...
  // 修复后：用 rejects 匹配器替代 try/catch，消除条件性 expect
  await expect(form.validate()).rejects.toEqual(defaultError.filter(item => item.address === "bb"));
