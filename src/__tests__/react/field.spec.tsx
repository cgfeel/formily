...
test("render field", async () => {
  ...
  fireEvent.change(screen.getByTestId("aa"), {
    target: { value: "123" },
  });

  // 字段 kk 没有调用监听函数，不会触发执行
  fireEvent.change(screen.getByTestId("kk"), {
    target: { value: "123" },
  });

  expect(onChange).toHaveBeenCalledTimes(1);
  expect(screen.getByTestId("bb-children")).toBeVisible();
  expect(screen.getByTestId("dd-children")).toBeVisible();

  // 被隐藏的字段需要通过 queryByTestId，使用 getByTestId 获取节点找不到会抛出错误
  expect(screen.queryByTestId("ee")).toBeNull();
...
test("useFormEffects", async () => {
  ...
  expect(screen.queryByTestId("custom-value")?.textContent).toEqual("");
  act(() => {
    form.query("aa").take(target => isField(target) && target.setValue("123"));
  });

  await waitFor(() => {
    expect(screen.queryByTestId("custom-value")?.textContent).toEqual("123");
  });
});
...
test("content", async () => {
  ...
  await waitFor(() => {
    expect(screen.queryByTestId("custom-field")).toBeVisible();
  });

  await waitFor(() => {
    expect(screen.queryByTestId("bb")).toBeVisible();
  });
  ...
  await waitFor(() => {
    expect(screen.queryByTestId("custom-read")?.innerHTML).toEqual(
      "read pretty text-component custom-field",
    );
  });

  await waitFor(() => {
    expect(screen.queryByTestId("input-read")?.innerHTML).toEqual("123-");
  });
});
...
test("field unmount and validate", async () => {
  ...
  // 这个时候 input 还是存在的
  expect(screen.queryByTestId("parent.child")).toBeVisible();
  ...
  expect(screen.queryByTestId("parent.child")).toBeNull();
  expect(screen.queryByTestId("unmounted")).toBeVisible();
