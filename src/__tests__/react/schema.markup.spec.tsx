...
describe("markup schema field", () => {
  ...
  test("void", () => {
    ...
    expect(screen.queryByTestId("void-component")).toBeVisible();
  });
  ...
  test("other", () => {
    ...
    expect(screen.queryByTestId("text-component")).toBeVisible();
  });
  ...
  test("no parent", () => {
    ...
    expect(screen.queryAllByTestId("input").length).toBe(1);
    expect(screen.queryByTestId("input")).toBeVisible();
  });
  ...
  test("props children", () => {
    ...
    expect(screen.queryByTestId("children-test")?.innerHTML).toEqual("props");
  });
...
describe("recursion field", () => {
  ...
  test("onlyRenderProperties", () => {
    ...
    expect(screen.queryAllByTestId("input").length).toEqual(3);
    expect(screen.queryAllByTestId("object").length).toEqual(1);
    expect(screen.queryAllByTestId("only-properties").length).toEqual(2);
  });
  ...
  test("mapProperties", () => {
    ...
    expect(screen.queryAllByTestId("input").length).toBe(3);
    expect(screen.queryAllByTestId("input")[0].getAttribute("value")).toEqual("123");
    expect(screen.queryAllByTestId("input")[1].getAttribute("value")).toEqual("123");
    expect(screen.queryAllByTestId("input")[2].getAttribute("value")).toEqual("");
  });
  ...
  test("filterProperties", () => {
    ...
    expect(screen.queryAllByTestId("input").length).toBe(1);
    expect(screen.queryAllByTestId("object").length).toBe(2);
  });
  ...
  test("onlyRenderSelf", () => {
    ...
    expect(screen.queryAllByTestId("input").length).toBe(0);
    expect(screen.queryAllByTestId("object").length).toBe(1);
  });
  ...
  test("schema reactions", async () => {
    ...
    expect(screen.queryByTestId("bbb")).toBeNull();

    const aaa = screen.queryByTestId("aaa");
    aaa && fireEvent.change(aaa, { target: { value: "123" } });
    await waitFor(() => {
      expect(screen.queryByTestId("bbb")).toBeVisible();
    });

    expect(screen.queryByTestId("ccc")).toBeNull();

    const bbb = screen.queryByTestId("bbb");
    bbb && fireEvent.change(bbb, { target: { value: "123" } });
    await waitFor(() => {
      expect(screen.queryByTestId("ccc")).toBeVisible();
    });
  });
  ...
  test("expression scope", async () => {
    ...
    await waitFor(() => expect(screen.queryByTestId("aa")).toBeVisible());
    expect(info.aa).toBeTruthy();

    await waitFor(() => expect(screen.queryByTestId("bb")).toBeVisible());
    expect(info.bb).toBeTruthy();

    await waitFor(() => expect(screen.queryByTestId("cc")).toBeVisible());
  ...
  test("expression x-visible", async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByText("BBB")).toBeVisible();
    });
  });
  ...
  test("expression x-value", async () => {
    ...
    await waitFor(() => {
      const bbb = screen.queryByPlaceholderText("bbb text");
  ...
  test("nested update component props with expression", async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByText("xxx")).toBeVisible();
    });
  ...
  test("schema x-reactions when undefined", async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId("select")).toBeVisible();
    });
  });
  ...
  test("void field children", async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId("btn")?.textContent).toEqual("placeholder");
    });
  });
  ...
  test("x-reactions runner for target", async () => {
    ...
    // 核心修改：拆分waitFor，每个仅包含一个断言
    await waitFor(() => {
      expect(screen.queryByTestId("btn")?.textContent).toEqual("Click 123");
    });
  ...
  test("multi x-reactions isolate effect", async () => {
    ...
    // 核心修改：拆分waitFor，每个仅包含一个断言
    await waitFor(() => {
      expect(screen.getByTestId("btn").textContent).toEqual("Click 123");
    });
    ...
    await waitFor(() => {
      expect(screen.getByTestId("btn").textContent).toEqual("Click 321");
    });
  ...
  test("nested record scope", async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId("text")?.textContent).toEqual("12332121");
    });
  });
  ...
  test("literal record scope", async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId("text")?.textContent).toEqual("1232");
    });
  });
  ...
  test("records scope", async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId("text")?.textContent).toEqual("3");
    });
  });
  ...
  test("propsRecursion as true", () => {
    ...
    expect(screen.queryAllByTestId("input").length).toBe(0);
    expect(screen.queryAllByTestId("object").length).toBe(1);
    expect(screen.queryAllByTestId("text").length).toBe(1);
  });
