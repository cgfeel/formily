...
test("reset", async () => {
  const form = attach(
    createForm<Partial<Record<string, number | null>>>({
...
test("match", () => {
  const form = attach(
    createForm<Partial<Record<string, number>>>({
...
test("required/setRequired", () => {
  ...
  const cc = attach(
    form.createField({
      name: "cc",
      validator: ["date", { max: 3 }, { required: true }],
    }),
  );
...
test("array path calculation with void index and void wrapper", async () => {
  ...
  const array_in_layout = attach(
    form.createArrayField({ basePath: "layout", name: "array_in_layout" }),
  );
...
test("field submit with error", async () => {
  ...
  // 核心修改：用 rejects 断言异步操作抛出异常（无条件执行 expect）
  await expect(childForm.submit(onSubmit)).rejects.toBeDefined();
...
test("field name is length in initalize", () => {
  ...
  attach(form.createObjectField({ name: "obj" }));
  const array = attach(
    form.createArrayField({ basePath: "obj", initialValue: [1, 2], name: "length" }),
  );
...
test("query value with sibling path syntax", () => {
  ...
  attach(
    form.createField({
      ...
      name: "input",
      reactions: [
        field => fn(field.query(".textarea").value(), field.query(".textarea").initialValue()),
      ],
    }),
  );
...
test("empty string or number or null value need rewite default value", () => {
  const form = attach(
    createForm<Partial<Record<string, number | string | null>>>({
