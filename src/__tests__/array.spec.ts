...
test("nest array remove", () => {
  ...
  const attr00 = attach(
    form.createField({ basePath: "metrics.0.content.0", initialValue: "123", name: "attr" }),
  );
  const attr10 = attach(
    form.createField({ basePath: "metrics.1.content.0", initialValue: "123", name: "attr" }),
  );
...
test("indexes: nest path need exclude incomplete number", () => {
  ...
  const objPathIncludeNum = attach(
    form.createField({ basePath: "metrics.0.a.10.iconWidth50", name: "attr" }),
  );
...
test("record: find array nest field record", () => {
  ...
  const form1 = attach(
    createForm({
      initialValues: {
        array: [{ a: [{ c: [1, 2], d: [1, 2] }] }, { a: [{ c: [1, 2], d: [1, 2] }] }],
      },
    }),
  );
