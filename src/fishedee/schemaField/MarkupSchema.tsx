...
const MarkupSchema: FC = () => (
    <FormBase
        footer={
            <div>
                <p>
                    和 1: <code>Field</code> 实践案例不同的是，在 <code>schema</code> 中 <code>ArrayField</code>{" "}
                    中的组件是通过 <code>useFieldSchema</code> + <code>RecursionField</code> 来渲染数组字段中的{" "}
                    <code>item</code> （在 <code>MarkupSchema</code> 中 <code>children</code> 第一个是 <code>item</code>
                ...
                <ul>
                    ...
                    <li>
                        ...
                        创建响应字段，并在渲染过程中包裹在指定 <code>schema</code> 节点 <code>ContextProvider</code> 下
                    </li>
                    <li>
                        而 <code>Field</code> 只作为收集字段属性以及渲染组件和渲染包装组件，如果是在 <code>schema</code>{" "}
                        模式下，这部分属性通过 <code>SchemaField</code> 转义并以 <code>props</code>、
                        <code>context</code> 的形式传递过来
                    </li>
                    <li>
