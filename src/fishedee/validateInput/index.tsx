...
const ValidateInput: FC = () => (
  <Wrapper form={form} header={<h2>core.3.1.1 值变更触发</h2>}>
    <Line title="设置 name">
      ...
      <button
        onClick={() => form.query("name").take(field => isField(field) && field.onInput("123"))}
      >
        input: "123"
      </button>
    </Line>
    <Line title="设置 name2">
      <button
        onClick={() => form.query("name2").take(field => isField(field) && field.onInput(""))}
      >
        input: ""
      </button>
      <button
        onClick={() => form.query("name2").take(field => isField(field) && field.onInput("123"))}
      >
        input: "123"
      </button>
    </Line>
