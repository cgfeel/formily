    ...
    footer={
      <div>
        ...
        <p>
          ...
          解构不同但是数据匹配路径相同。这说明了 <code>VoidField</code>{" "}
          的路径可以忽略直接由上一级进行匹配。
        </p>
        <p>
          做这个研究的目的是由 <code>ArrayTable</code> 这个组件想到，每一个{" "}
          <code>ArrayTable.column</code> 都是一个 <code>VoidField</code>，它的父级是{" "}
          <code>ArrayField.item</code>，它的子集是 <code>Field</code>，设置或不设置{" "}
          <code>ArrayTable.column</code> 的 <code>name</code>
          ，都不影响内部字段的路径，也不影响其赋值
        </p>
        <p>
