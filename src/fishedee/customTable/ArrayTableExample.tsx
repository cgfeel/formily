      ...
      <div>
        ...
        <ul>
          <li>
            ...
            <code>antd table</code>，因为考虑到了列表自增，采用 <code>ArrayBase</code>{" "}
            更符合实际开发
          </li>
        ...
        <ul>
          <li>
            创建一个自定义组件，将其通过 <code>observer</code> 包裹，并通过{" "}
            <code>ArrayBase.mixin</code> 绑定在 <code>ArrayBase</code> 对象中
          </li>
          <li>
            如果有需要提供对外渲染组件，如：<code>Addition</code>、<code>Remove</code>、
            <code>Copy</code>等
          </li>
          <li>
            提供内部 <code>schema</code> 渲染逻辑，例如通过 <code>schema.reduceProperties</code>{" "}
            遍历 <code>schema</code>、通过 <code>RecursionField</code> 渲染过滤的{" "}
            <code>schema</code> 节点
          </li>
        </ul>
        <p>
          除此之外对于 <code>ArrayTable</code> 的模型要了解 <code>antd table</code>{" "}
          的特性，可以先分成 2 个部分：
        </p>
        <ul>
          <li>
            首先是表格头部栏目，但在 <code>antd</code> 中并没有提供类似 <code>th</code>{" "}
            这样的标签作为受控
          </li>
          <li>
            其次就是单元格，在 <code>antd</code> 中可以通过 <code>column</code> 中的{" "}
            <code>render</code> 为每个单元格包裹一个 <code>context</code>
          </li>
        </ul>
        <p>
        ...
        <ul>
          ...
          <li>
            ...
            <code>reactions</code>，做出相应结果；这时提供给 <code>Table</code> 组件的{" "}
            <code>column</code> 为空
          </li>
          <li>
            重新渲染再次调用 <code>compileColumns</code> 去计算 <code>column</code>，由于{" "}
            <code>schema</code> 已挂载，所以 <code>display</code> 这个属性即默认的{" "}
            <code>visible</code>
          </li>
          <li>
            拿到上述结果计算出要展示的 <code>column</code>，演示中通过{" "}
            <code>compileTableSource</code>
          </li>
        </ul>
        <p>注意事项：</p>
        <ul>
          <li>
            表头受控通过外部渲染对应的 <code>schema</code>，这个时候 需要设置{" "}
            <code>RecursionField</code> 属性 <code>onlyRenderSelf</code>，仅渲染自身，其路径是：
          ...
          <li>
            对于没有指定 <code>id</code> 的 <code>dataSource</code>，需要通过 <code>rowKey</code>{" "}
            指定
          </li>
          <li>
