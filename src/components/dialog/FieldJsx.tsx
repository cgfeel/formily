...
const FieldInner: FC = () => (
    <>
        {fieldKeys.map((key, i) => (
            <Field component={[Input]} decorator={[FormItem]} key={key} name={key} title={`输入框${i + 1}`} required />
        ))}
    </>
);
    ...
    <Pannel
        footer={
            <div>
                ...
                <ul>
                    <li>
                        快速打开，关闭能力：通过<code>IFormDialog</code>对象触发<code>open</code>和<code>close</code>
                    </li>
                    <li>
                        中间件能力，自动出现加载态：
                        <ul>
                    ...
                    <li>
                        渲染函数内可以响应式能力：通过<code>schema</code>和<code>Field</code>实现表单渲染、响应、交互
                    </li>
                    <li>
