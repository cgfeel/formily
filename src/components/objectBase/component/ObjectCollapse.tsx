...
const isObject = (data: any): data is DataType =>
  data instanceof Object && data.constructor === Object;
...
const RenderAddition: FC<Omit<RenderItemsProps, "dataSource" | "prefix">> = ({
  schema,
  path = "",
}) =>
  schema.reduceProperties((addition, schema, key) => {
    return isAdditionComponent(schema) ? (
      <RecursionField schema={schema} name={`${path}.${key}`} />
    ) : (
      addition
    );
  }, null);
...
const RenderOps: FC<Omit<RenderItemsProps, "dataSource" | "prefix">> = ({ schema, path = "" }) => {
  return schema.reduceProperties((ops, schema) => {
    return isOpsComponent(schema) ? (
      <RecursionField schema={schema} name={[path, "ops"].join(".")} />
    ) : (
      ops
    );
  }, null);
...
const RenderRemove: FC<Omit<RenderItemsProps, "dataSource" | "prefix">> = ({ schema, path = "" }) =>
  schema.reduceProperties((addition, schema, key) => {
    return isRemoveComponent(schema) ? (
      <RecursionField schema={schema} name={`${path}.${key}`} />
    ) : (
      addition
    );
