        ...
        footer={
            <div>
                <p>
                    ...
                    <a href="https://core.formilyjs.org/zh-CN/api/models/form#createvoidfield">createVoidField</a> React
                    实现，它是专门用于将 <code>ViewModel</code>{" "}
                    与虚拟布局控件做绑定的桥接组件，可以用来控制数据型字段的显示隐藏，交互模式等，<code>VoidField</code>{" "}
                    组件属性参考{" "}
                ...
                <p>
                    该例子演示了如何用 <code>VoidField</code> 控制子节点显示隐藏，注意观察，<code>VoidField</code>{" "}
                    隐藏的时候，子节点的数据会同时被清空，因为 <code>visible</code> 为 <code>false</code> 代表{" "}
                    <code>display</code> 为 <code>none</code>，这种隐藏是不会保留字段值的。
                </p>
                <p>
