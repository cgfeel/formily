        ...
        footer={
            <div>
                <p>
                    ...
                    实例注入副作用逻辑，用于实现一些较为复杂的场景化组件，可以看作组件中使用 <code>createForm</code>{" "}
                    中的 <code>effect</code>
                </p>
                <p>注意：</p>
                <ol>
                    <li>
                        在 <code>effects</code> 函数内监听 <code>onFormInit</code> 是无效的，因为渲染到当前组件，
                        <code>Form</code> 早已初始化
                    </li>
                    <li>
                        同时 <code>effects</code> 函数是只会执行一次，所以想要依赖 <code>useState</code> 的数据，请使用{" "}
                        <code>useRef</code> 的引用数据
                    </li>
                    <li>
